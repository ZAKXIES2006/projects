...
#include "Validation.h"      // Corrected path and renamed from utility.h
...
LibrarySystem::LibrarySystem() {
  ...
  if (findAccountByID("admin") == nullptr) {
      ...
      accounts_.push_back(std::make_unique<Admin>("admin", "password"));
  }
}
...
void LibrarySystem::showAdminMenu() {
  ...
  switch (choice) {
    ...
    case 3: adminViewAllMembers(); break;
...
void LibrarySystem::showMemberMenu() {
  ...
  int choice = util::getValidatedInt("Enter choice: ");
  switch (choice) {
      case 1: memberBorrowBook(); break;
      case 2: memberReturnBook(); break;
      case 3: memberViewMyBooks(); break;
      case 4: handleLogout(); break;
      default: std::cout << "Invalid choice. Please try again.\n";
  }
}
...
void LibrarySystem::handleLogin() {
  ...
  if (user != nullptr && user->checkLogin(id, pw)) {
    current_user_ = user;
...
void LibrarySystem::adminAddBook() {
  ...
  books_.push_back(std::make_unique<Book>(isbn, title, author));
...
void LibrarySystem::adminAddMember() {
  ...
  int type = util::getValidatedInt("Enter account type (1=Standard, 2=Premium): ");
  if (type == 2) {
...
void LibrarySystem::memberBorrowBook() {
  std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
  std::string isbn = util::getNonEmptyString("Enter ISBN of the book to borrow: ");
  try {
    Book* book = findBookByISBN(isbn);
    if (book == nullptr) throw std::runtime_error("Book not found.");
    if (!book->isAvailable()) throw std::runtime_error("Book is already borrowed by someone else.");
...
void LibrarySystem::memberReturnBook() {
  std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
  std::string isbn = util::getNonEmptyString("Enter ISBN of the book to return: ");
  try {
    Book* book = findBookByISBN(isbn);
    if (book == nullptr) throw std::runtime_error("Book not found in the system.");
    Member* current_member = dynamic_cast<Member*>(current_user_);
...
void LibrarySystem::recordTransaction(const std::string& type, Book* book, Member* member) {
...
void LibrarySystem::saveBooks() const {
  ...
  if (!file) { std::cerr << "Error: Could not open " << BOOKS_FILE << " for writing.\n"; return; }
  for (const auto& book : books_) {
...
void LibrarySystem::saveAccounts() const {
  ...
  if (!file) { std::cerr << "Error: Could not open " << ACCOUNTS_FILE << " for writing.\n"; return; }
  for (const auto& acc : accounts_) {
...
void LibrarySystem::saveTransactions() const {
    ...
    if (!file) { std::cerr << "Error: Could not open " << TRANSACTIONS_FILE << " for writing.\n"; return; }
