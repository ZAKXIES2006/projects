...
void Member::displayDetails() const {
  ...
  if (currently_borrowed_.empty()) { // If no books borrowed
    ...
    for (const auto* book : currently_borrowed_) { // Loop through every book in currently_borrowed
...
bool Member::borrowBook(Book* book) {
  if (currently_borrowed_.size() >= kMaxBooks_) { // If member has reached their borrowing capacity
    return false;
  }
  // Otherwise proceed
...
bool Member::returnBook(Book* book) {
  // Find and store an iterator pointing to the book's position in currently_borrowed_
  auto it = std::find(currently_borrowed_.begin(), currently_borrowed_.end(), book);
  if (it != currently_borrowed_.end()) { // if the iterator does not equal ned(), book was found currently_borrowed_
...
bool Member::hasBorrowedBook(const std::string& isbn) const {
    for (const auto* book : currently_borrowed_) { // Loop through every book in currently_borrowed_
...
std::string Member::toFileString() const {
    std::stringstream ss;
    ss << getAccountType() << "|" << user_id_ << "|" << password_ << "|"; // Format of the string stream
    for (size_t i = 0; i < currently_borrowed_.size(); ++i) { // Loop through borrowed books
